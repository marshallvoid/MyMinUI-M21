...
# Boxart locations (Fancy mode, in lookup order)

- ROMs: `/Roms/<EMU>/Imgs/<game>.png`
  (multi-disc/collated folder with `<game>.m3u` inside resolves the same way)
- emulator folders: `/Roms/<EMU>/Imgs/<EMU>.png`
- tool paks: `/Tools/m21/<pak>/Imgs/<pak>.png`
- system folders (Recently Played, Favorites, HiddenRoms, Collections):
  `/Imgs/<folder>.png` (e.g. `/Imgs/Favorites.png`, `/Imgs/Collections.png`)
  (the Tools folder no longer exists at root -- tool paks now live in the Y/Options menu;
  no per-folder `Imgs/` lookup -- collection subfolders like `/Collections/Pokemon`
  also resolve here, then to `default.png`)
- final fallback for everything: `/Imgs/default.png`
